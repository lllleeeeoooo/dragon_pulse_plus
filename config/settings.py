...
class Settings(BaseSettings):
    ...
    TAIL_GAME_OPEN_GAP_PCT: float = Field(default=2.0, description="尾盘博弈卖出：次日开盘涨幅≥此比例(%)视为高开，用当日最高价兑现")
    TAIL_GAME_NEAR_LIMIT_RATIO: float = Field(default=0.92, description="尾盘博弈候选：涨幅≥涨停线×此比例的涨停附近放量下限")
    TAIL_GAME_VOL_RATIO: float = Field(default=3.0, description="尾盘博弈候选：涨停附近放量的量比下限")
