...
```text
[08:30 盘前简报] ➔ [09:26 竞价观察] ➔ [09:30-15:00 15秒轮询&AI实盘] ➔ [15:30 盘后深度复盘]
```
...
### 2. 09:26 竞价观察 (Call Auction)
- 获取 09:25 全市场集合竞价撮合快照。
- 自动从数据库提取上一交易日复盘推荐的标的，计算**竞价开盘涨幅**与**竞价成交额占昨日全天的百分比**。
- 计算全市场昨日涨停股今日竞价开盘平均溢价率。
- LLM 评估今日竞价风格，生成【竞价直接挂单买入】/【开盘观察分时再定】/【放弃介入】指令。

### 3. 09:30-15:00 盘中 15 秒实时轮询与 AI 自动持仓 (Market Monitor)
- **15 秒全市场快照监控**：
  - **点火预警**：扫描成交量比 $\ge 3.0$ 且单次涨幅 $\ge 3.0\%$ 的异动标的。
  - **全市场炸板预警**：扫描高位股炸板分歧，防范天地板。
- **AI 自动买入流转**：
  - 若异动标的为**复盘推荐的观察标的**，且**开盘未死封一字板**（一字板视为买不进），系统自动将其纳入 **AI 自动持仓 (`AI_AUTO`)**，计算成本价并开始实时追踪。
- **持仓风控与 AI 自动卖出**：
  - 实时计算所有持仓（手动与 AI 自动）的当前价格与**实时收益率 (%)**。
  - 触发龙头断板或跌破 5 日线时，向手机发送告警；对 `AI_AUTO` 自动持仓执行平仓出局。

### 4. 15:30 盘后深度复盘 (Post-Market)
- 统计当日涨跌停数、炸板率、最高连板梯队分布及成交额 Top 20。
- 计算 6D 情绪多维向量，定性当前情绪周期。
- 自动提取上一交易日推荐标的进行**胜率复盘**。
- 自动计算今日**动态中军池**与高位龙头**交易所监管异动残余空间**。
- 龙虎榜游资席位画像分析（格局派/砸盘派/量化派）。
- LLM 生成次日详细操作指南（分场景给出开盘幅度、竞价量能比、买卖/止损条件）并保存推荐标的至数据库。
...
系统提供 5 种量化卖出风控场景，覆盖标的盘口与全市场环境：

1. 💥 **场景 1：龙头断板必卖**（`CRITICAL` 级）
   - 标的今日曾封涨停，但盘中炸板且收盘前未能重新封板。断板必卖是短线第一铁律。
2. 📉 **场景 2：均线双重破位止损**（`HIGH` 级）
   - 现价跌破分时均价线（VWAP）且跌破 5 日移动平均线（MA5）。
3. 🎯 **场景 3：策略止盈止损线触发**（`HIGH` 级）
   - 收益率达到策略推荐设定的止盈位或硬止损位（如 $-5\%$）。
4. 🚨 **场景 4：全市场情绪到顶预警**（`WARNING` 级）
   - 全市场最高连板高度 $\ge 8$ 板且全市场炸板率 $> 35\%$，预示退潮砸盘临近。
5. 🏛️ **场景 5：交易所监管严重异动红线触发**（`HIGH` 级）
   - 10 日偏离度近 $100\%$ 或异动已达 3 次，防范停牌核查与资金砸盘规避。
...
## 八、GET HTTP 持仓管理 API 接口

随系统启动在端口 **`8000`** 后台运行，可通过浏览器或手机快捷指令使用：
...
# ==================== 6. 策略引擎参数配置 ====================
CORE_POOL_MIN_BETA=0.8
CORE_POOL_MIN_AMOUNT=20.0
SECOND_WAVE_RETREAT_MIN=0.30
SECOND_WAVE_RETREAT_MAX=0.50
```
...
```
启动后系统将：
- 自动在后台启动持仓管理 Web API 服务 (`http://127.0.0.1:8000`)；
...
│   ├── strategies.py        # 五大战法标签化归因引擎
│   ├── seat_analyzer.py     # 龙虎榜游资席位画像库
│   ├── holding_monitor.py   # 持仓风控卖出条件检测
