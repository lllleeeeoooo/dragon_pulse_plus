...
- 龙虎榜席位画像分析（格局派/砸盘派/散户派/对倒派/外资北向）：六一中路等名席位人工标签优先，新营业部按最近实际买卖行为自动分类更新，可直接在 `/monitor` 看板查看。
...
### 4. 龙虎榜席位画像 `/data/seats`
- **URL**: `http://127.0.0.1:8000/data/seats?top=20`（按累计净买入降序）
- **参数**: `seat_type` 可选 `格局派/砸盘派/散户派/对倒派/外资北向/未知`；`top` 条数；`active_only`
- **统计**: `http://127.0.0.1:8000/data/seats/stats`（席位总数 + 按类型计数）
- **说明**: 席位分类自动更新——名席位人工标签优先，新营业部按实际买卖行为定型（需累计 3 天上榜）
