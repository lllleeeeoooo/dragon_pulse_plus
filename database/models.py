...
class Holding(Base):
    ...
    change_pct = Column(Float, default=0.0, comment="个股当日市场涨跌幅(%)，监控每15秒从行情快照更新")
    decision_source = Column(String(8), default="rule", comment="B方案: 买入决策来源 llm/rule(LLM失败降级)")
...
class DatabaseManager:
    ...
    def create_tables(self):
        ...
        _ensure_column(self.engine, "holdings", "change_pct", "FLOAT DEFAULT 0.0")
        _ensure_column(self.engine, "holdings", "decision_source", "VARCHAR(8) DEFAULT 'rule'")
