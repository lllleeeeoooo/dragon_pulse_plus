...
from core.trade_calendar import is_trading_day, is_last_non_trading_day, get_previous_trading_day, get_n_trading_days_ago
...
def _record_job_run(job_id: str, job_name: str, state: str = "", progress: str = ""):
    """记录定时任务执行时间（仅数据库，不占内存）；state/progress 供看板显示任务状态与进度。
    detail 格式: HH:MM:SS|job_id[|state][|progress]——_get_job_status 按 '|' 解析。"""
    ...
    if progress:
        detail += f"|{progress}"
    try:
        # 高频/长时任务每轮更新同一条记录（避免重复写入）
        if job_id in _UPDATE_IN_PLACE_JOBS:
...
def _get_job_status() -> List[Dict[str, Any]]:
    ...
    all_jobs = [
        ...
        {"id": "job_dragon_expire",  "name": "龙头过期标记",     "time": "04:05", "desc": "超过30个交易日无人气龙头自动失效"},
        {"id": "job_pre_market",     "name": "盘前简报",         "time": "08:30", "desc": "新闻抓取→LLM分析→推送题材预测"},
        {"id": "job_data_check",     "name": "盘前数据检查",     "time": "08:15", "desc": "核对上一交易日盘后数据是否齐全，缺失 Bark 告警"},
    ...
    try:
        ...
        for log in logs:
            ...
            if "|" in detail:
                parts = detail.split("|")
                db_records[parts[1]] = {
                    "last_run": parts[0], "last_date": today,
    ...
    for j in all_jobs:
        ...
        j["status"] = record["status"] if record and record.get("status") else ""
        j["progress"] = record["progress"] if record and record.get("progress") else ""
    return all_jobs
...
def _auto_populate_dragons(trade_date: str, zt_df):
    ...
    try:
        session = db_manager.get_session()
        try:
            # 30 个交易日前的龙头标记为失效（与 04:05 龙头过期任务同一口径，防两处 cutoff 不一致）
            cutoff = get_n_trading_days_ago(
                30, datetime.datetime.strptime(trade_date, "%Y%m%d").date())
